        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-6 lg:px-8">
              ...
              l.hasMenu ? (
                ...
                >
                  ...
                  >
                    {l.label}
                    <ChevronDown
                      className={`h-3.5 w-3.5 ${ menuOpen ? 'rotate-180' : '' }`}
                    />
                  </NavLink>

                  {/* Mega-menú de agentes */}
                  <div
                    className={`absolute left-1/2 top-full z-50 w-[min(92vw,30rem)] -translate-x-1/2 pt-4 ${ menuOpen ? 'visible translate-y-0 opacity-100' : 'invisible -translate-y-1 opacity-0' }`}
                  >
                    <div className="overflow-hidden rounded-none bg-paper">
                        ...
                        {AGENTS.map((a) => {
                          return (
                            <Link
                              key={a.slug}
                              to={`/agentes/${a.slug}`}
                              className="group flex flex-col bg-paper p-4"
                            >
          ...
          <div className="hidden items-center gap-6 md:flex">
            ...
            <a
              ...
              className={`group inline-flex items-center gap-1.5 rounded-none px-4 py-2 text-sm font-medium ${ !isLightText ? 'bg-ink text-paper' : 'bg-paper text-ink' }`}
            >
              Agendar demo
